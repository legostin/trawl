...
mod tests {
    ...
    async fn rewrite_path_string_and_regex_keep_query() {
        let res = run(
            "rewritePath(request, '/v3/', '/v4/'); rewritePath(request, /items/, 'ads');",
            r#"{"request":{"headers":{},"url":"https://h.kz/v3/items/rec?limit=1","host":"h.kz","path":"/v3/items/rec?limit=1"}}"#,
        )
        .await;
        assert_eq!(res.action, "continue", "err: {:?}", res.error);
        let req = res.request.unwrap();
    ...
    async fn path_segments_decode_and_skip_query() {
        let res = run(
            ...
            r#"{"request":{"headers":{},"path":"/v3/items/%D0%B0?x=1"}}"#,
        )
        .await;
        assert_eq!(res.action, "continue", "err: {:?}", res.error);
        assert_eq!(res.request.unwrap()["__s"], json!(["v3", "items", "а"]));
    }
