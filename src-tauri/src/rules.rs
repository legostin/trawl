...
mod tests {
    ...
    fn expands_project_variables_in_patterns() {
        ...
        assert_eq!(
            expand_pattern("{{APP_HOST}}/v4/items*", &env),
            "app.example.com/v4/items*"
        );
    ...
    fn glob_match_env_ignores_leading_scheme() {
        ...
        assert!(glob_match_env(
            "https://app.example.com/v3/items/recommendation*",
            "app.example.com/v3/items/recommendation?page=1",
            &env,
        ));
